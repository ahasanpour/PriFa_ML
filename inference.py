...
from train import MemModule_sam
...
def main(argv):
    ...
    def load(arch):
        return MemModule_sam(network(arch), nclass=100 if FLAGS.dataset == 'cifar100' else 10,
    ...
    xs_all = np.load(os.path.join(FLAGS.logdir_main,"x_gray_train.npy"))[:FLAGS.dataset_size]
    ys_all = np.load(os.path.join(FLAGS.logdir_main,"y_train.npy"))[:FLAGS.dataset_size]
...
if __name__ == '__main__':
    ...
    flags.DEFINE_string('logdir_main', 'experiments/', 'Directory where to save checkpoints and tensorboard data.')
