...
class TrainLoop(objax.Module):
    ...
    def train_step(self, summary: Summary, data: dict, progress: np.ndarray, lr_cos):
        ...
        kv = self.train_op(progress, data['image'].numpy(), data['label'].numpy(), lr_cos)
        for k, v in kv.items():
            if jn.isnan(v):
...
class MemModule_sam(TrainLoop):
    def __init__(self, model: Callable, nclass: int, mnist=False, rho=0.0, eps=1e-12, **kwargs):
        """
        Completely standard training. Nothing interesting to see here.
        """
        super().__init__(nclass, **kwargs)
        ...
        def get_sam_gradient1(model, g, x,y):
            ...
            assert len(g) == len(self.train_vars), 'Expecting as many gradients as trainable variables'
...
if __name__ == '__main__':
    ...
    flags.DEFINE_bool('dp', True, 'Use tune name?')
    app.run(main)
